class LiveSession {
  ...
  _sendChat({ text, target, from, fromPlayerId }) {
    if (!text) return;
  ...
  _handleMessage({ data }) {
    ...
    switch (command) {
      ...
      case "pronouns":
        ...
        break;
      case "chat":
        this._store.dispatch("chat/receive", params);
        break;
    }
  }
    ...
    switch (type) {
      case "session/setSessionId":
        if (state.session.sessionId) {
          ...
          session.disconnect();
          session._store.commit("chat/clear");
        }
        break;
      ...
      case "players/update":
        ...
        break;
      case "chat/sendToSocket":
        session._sendChat(payload);
        break;
    }
  });
