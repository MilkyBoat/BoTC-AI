...
`src/domain/protocol/` 是房主浏览器中的权威状态与事件底座，不依赖 Vue、Vuex、WebSocket 或 LLM。当前协议版本为 `0.5.0`：`game.create` 精确绑定 M1-R1 规则集和一个不可变角色规则包，`src/domain/rules/` 提供基础阶段、生死、提名投票、流放、处决与常规胜负，`src/domain/abilities/` 提供能力实例、触发队列、效果生命周期和结构化说书人裁量。仓库既保留不含具体角色的默认框架包，也提供 `src/domain/rulesets/trouble-brewing/` 中完整的《暗流涌动》规则包。
...
`game.start` 的席位可选携带真实角色、感知角色、阵营和角色实例 ID；命令、`game.started` 事件与权威状态还可携带严格的 `troubleBrewing` 子状态。空框架对局可不提供这些字段；协议不提供任意自由写扩展容器。
...
角色包运行时可以只读声明 `validateGameStart`、`handlePhaseAdvance`、`handleKill` 和 `handleExecution` 四个白名单钩子。开局钩子只能返回空值或严格领域错误；其余钩子返回空值时继续基础逻辑，返回标准命令处理结果时接管当次结算。基础不变量仍严格验证常规胜负；圣徒和镇长特殊胜负由角色包扩展不变量继续验证。
...
- 恢复日志时必须再次提供与事件流身份完全相同且校验通过的角色规则包；恢复只归约已记录事件，不重新运行处理器或反应器。`0.1.0` 至 `0.4.0` 事件流不会被 `0.5.0` 静默补充角色真相、剧本状态或结算语义。
