...
`src/domain/protocol/` 是房主浏览器中的权威状态与事件底座，不依赖 Vue、Vuex、WebSocket 或 LLM。当前协议版本为 `0.3.0`：`game.create` 固定 M1-R1 规则范围身份并进入准备阶段，`src/domain/rules/` 在同一命令总线上提供开局、首夜/昼夜、基础生死、复活、提名、顺时针投票、死亡票、最高票候选、旅行者流放、常规处决和常规胜负。具体角色、特殊胜负和 UI 接入仍由后续 M1 需求实现。
...
- 恢复包含扩展事件的日志时，必须提供与写入时相同的命令和事件定义版本；`0.1.0`、`0.2.0` 事件流不会被 `0.3.0` 静默补充死亡票或提名状态。
