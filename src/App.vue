<template>
  ...
  >
    ...
    <ChatPanel v-if="session.sessionId" />
...
export default {
  ...
  data() {
    return {
      aiDebugSyncTimer: null,
      version: pkg.version,
    };
  },
  mounted() {
    this.$store.dispatch("aiDebug/init");
    this.aiDebugSyncTimer = window.setInterval(() => {
      this.$store.dispatch("aiDebug/sync");
    }, 500);
  },
  beforeDestroy() {
    if (this.aiDebugSyncTimer) {
      window.clearInterval(this.aiDebugSyncTimer);
    }
  },
  methods: {
    keyup(event) {
