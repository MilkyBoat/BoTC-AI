  ...
  <div id="controls">
    ...
    <div class="menu" :class="{ open: grimoire.isMenuOpen }">
      ...
      <ul>
        ...
        <template v-if="tab === 'ai' && !session.isSpectator">
          <li class="headline">AI说书人</li>
          <li :class="{ disabled: !canRequestAiStart }" @click="requestAiStart">
            启动AI说书
            <em>{{ aiStatusText }}</em>
          </li>
          <li :class="{ disabled: !canForceAiStop }" @click="forceStopAi">
            强制停止
            <em>{{ canForceAiStop ? "可用" : "不可用" }}</em>
          </li>
          <li @click="toggleAiDebugPanel()">
            Debug面板
            <em>{{ aiDebugVisible ? "显示中" : "已隐藏" }}</em>
          </li>
          <li class="status-row">
            当前阶段
            <em>浏览器启动骨架已接入</em>
          </li>
          <li v-if="aiRuntime.runtimeMode" class="status-row">
            运行模式
            <em>{{ aiRuntime.runtimeMode }}</em>
          </li>
          <li class="status-row">
            交互适配器
            <em>{{ aiRuntime.interactionSubscriptions }}</em>
          </li>
          <li class="status-row">
            日志缓存
            <em>{{ aiRuntime.logCount }}</em>
          </li>
          <li v-if="aiRuntime.instanceId" class="status-row">
            运行实例
            <em>{{ aiRuntime.instanceId }}</em>
          </li>
          <li v-if="aiRuntime.startedAt" class="status-row">
            启动时间
            <em>{{ formatIntentTime(aiRuntime.startedAt) }}</em>
          </li>
          <li v-if="aiRuntime.stoppedAt" class="status-row">
            停止时间
            <em>{{ formatIntentTime(aiRuntime.stoppedAt) }}</em>
          </li>
          <li v-if="aiRuntime.lastStartIntentAt" class="status-row">
            最近点击
            <em>{{ formatIntentTime(aiRuntime.lastStartIntentAt) }}</em>
          </li>
          <li v-if="aiRuntime.lastError" class="status-row error">
            启动错误
            <em>{{ aiRuntime.lastError }}</em>
          </li>
        </template>
...
export default {
  computed: {
    ...
    ...mapState("aiDebug", ["visible"]),
    aiRuntime() {
      return {
        status: this.status,
        lastStartIntentAt: this.lastStartIntentAt,
    ...
    canForceAiStop() {
      return (
        !this.session.isSpectator &&
        (this.status === "starting" || this.status === "running")
      );
    },
    aiStatusText() {
      if (this.status === "running") return "正在运行";
      if (this.status === "starting") return "启动中";
      if (this.status === "stopped") return "已停止";
      if (this.status === "error") return "启动失败";
      return "未启动";
    },
  },
  data() {
  ...
  methods: {
    ...
    requestAiStart() {
      if (!this.canRequestAiStart) return;
      this.$store.dispatch("aiRuntime/requestStart");
    },
    forceStopAi() {
      if (!this.canForceAiStop) return;
      this.$store.dispatch("aiRuntime/requestStop");
    },
    toggleAiDebugPanel() {
      this.$store.commit("aiDebug/toggleVisible");
    },
    formatIntentTime(ts) {
