...
export default {
  ...
  methods: {
    ...
    labelClass(message) {
      return {
        host: message.from === "host" || message.from === "storyteller",
      };
    },
    formatPlayerLabel(playerId, seat) {
      if (seat > 0) {
        const seatPlayer = this.players[seat - 1];
        if (seatPlayer) {
          return seatPlayer.name || `${seat}`;
        }
      }
    ...
    formatSender(message) {
      if (message.from === "storyteller" && message.scope === "direct") {
        const targetLabel = this.formatPlayerLabel(
          message.target?.playerId,
          message.target?.seat,
        );
        return `说书人对${targetLabel}私聊`;
      }
      ...
      if (message.from === "storyteller") return "说书人";
