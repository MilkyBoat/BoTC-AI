...
function toAnthropicMessages(messages) {
  ...
  for (const m of messages) {
    ...
    if (type === 'ai') {
      ...
      result.push({ role: 'assistant', content: content.length > 0 ? content : (text || '') })
      continue
    }
  }
...
class ChatClaude extends BaseChatModel {
  constructor(fields = {}) {
    super(fields)
    this.model = fields.model || process.env.CLAUDE_MODEL || 'claude-sonnet-4-6'
    this.apiKey = fields.apiKey || process.env.CLAUDE_API_KEY || process.env.ANTHROPIC_API_KEY
    ...
    this.client = new Anthropic.default(clientOpts)
  }
