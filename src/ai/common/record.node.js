...
function nowISO() {
  return dayjs ? dayjs().toISOString() : new Date().toISOString();
}
...
function ensureSession() {
  ...
  wsAll = fs.createWriteStream(path.join(sessionDir, "all.log"), {
    flags: "a",
  });
  wsLLM = fs.createWriteStream(path.join(sessionDir, "llm.log"), {
    flags: "a",
  });
  wsGame = fs.createWriteStream(path.join(sessionDir, "game.log"), {
    flags: "a",
  });
  wsState = fs.createWriteStream(path.join(sessionDir, "state.log"), {
    flags: "a",
  });
  const rel = path.relative(process.cwd(), sessionDir);
  process.stdout.write(
    `[log] session=${name} dir=${rel} provider=${dayjs ? "dayjs" : "date"}\n`,
  );
  inited = true;
}
...
function writeLine(type, line) {
  try {
    wsAll && wsAll.write(line);
  } catch {}
  if (type === "llm") {
    try {
      wsLLM && wsLLM.write(line);
    } catch {}
  }
  if (type === "state") {
    try {
      wsState && wsState.write(line);
    } catch {}
  }
  if (type !== "llm" && type !== "tool" && type !== "script") {
    try {
      wsGame && wsGame.write(line);
    } catch {}
  }
}
