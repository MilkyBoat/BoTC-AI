...
class RoleAllocAgent {
  ...
  buildCandidates(script) {
    ...
    for (const r of roles) {
      if (byTeam[r.team])
        byTeam[r.team].push({ name: r.name, team: r.team, ability: r.ability });
    }
...
- 初始 tokens 请根据 setup 与开局信息最小化生成（如 "是酒鬼"、首夜信息等）`;
    const userPayload = {
      ...
      require: {
        format: {
          players: [{ seat: 1, knownRole: "", realRole: "", tokens: [] }],
        },
      },
    };
  ...
  async allocate({ playerCount, script, customRules, debug } = {}) {
    if (debug === true || (typeof window === "undefined" && process.env.DEBUG === "1")) {
