...
function getScriptsDir() {
  const path = getPath();
  return path.resolve(process.cwd(), "game_script");
}

async function listScripts() {
  const fs = getFs();
  try {
    const files = await fs.promises.readdir(getScriptsDir());
...
function parseScript(data) {
  const roles = data.filter(
    (x) => x && x.id && x.team && x.id !== "_meta" && x.team !== "traveler",
  );
  const meta = data.find((x) => x && x.id === "_meta") || {};
  const firstNight = roles
    .filter((r) => Number(r.firstNight) > 0)
    .sort((a, b) => Number(a.firstNight) - Number(b.firstNight));
  const otherNight = roles
    .filter((r) => Number(r.otherNight) > 0)
    .sort((a, b) => Number(a.otherNight) - Number(b.otherNight));
...
function renderScript(raw) {
  try {
    ...
    const roles = entries.filter(
      (x) => x && x.id && x.team && x.id !== "_meta",
    );
    const { nightOrder } = parseScript(entries);
    const getOrderNames = (list) =>
      Array.isArray(list) ? list.map((e) => String(e.name || e.id)) : [];
    const joinOrder = (names) =>
      names && names.length ? names.join(" -> ") : "无";
    const fmt = (list) =>
    ...
    for (const r of roles) {
      ...
      const remindersGlobal = Array.isArray(r.remindersGlobal)
        ? r.remindersGlobal
        : Array.isArray(r.reminders_global)
          ? r.reminders_global
          : [];
      if (reminders.length)
        lines.push(`  可选Token(绑定到特定座位): ${fmt(reminders)}`);
      if (remindersGlobal.length)
        lines.push(`  可选Token(全局，即座位0): ${fmt(remindersGlobal)}`);
    }
    lines.push(
      `首夜行动顺序: ${joinOrder(getOrderNames(nightOrder.firstNight))}`,
    );
    lines.push(
      `其他夜晚行动顺序: ${joinOrder(getOrderNames(nightOrder.otherNight))}`,
    );
  ...
  try {
    ...
    if (debug) {
      ...
      if (scripts.length === 0) {
        record(
          "error",
          "未在 ./game_script 发现剧本，请提供标准格式的示例json后重试",
        );
        return null;
      }
