...
主要来源为 `wiki.bloodontheclocktower.com` 等官方资料。相邻实验项目的中文 Wiki 抓取逻辑只作为页面分类与清洗需求的参考，不直接复制实现。
...
当前基础设施使用 Jest、Vue Test Utils v1 和 Vue CLI 转换链承载单元、Vue 组件与场景测试，使用 Playwright Chromium 承载浏览器端到端测试。Node.js 22、锁文件安装、分层 npm 脚本和 GitHub Actions 组成一致门禁；测试默认阻断外部网络，不把公共会话、真实 LLM 或生产数据作为依赖。初始源码不设置全仓覆盖率阈值，后续新模块在各自需求中定义有效覆盖和模块级门禁。
